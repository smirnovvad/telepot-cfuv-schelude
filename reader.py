...
print(list(kurs3.keys()))
msg = {'text': ''}
msg['text'] = '4ПКС8'
print(msg['text'][0])

with open('Расписание (4 курс) %s.csv' % (days[datetime.datetime.today().weekday()]), newline='') as csvfile:
    reader = csv.reader(csvfile, delimiter=';')
    shelude = ''
    for row in reader:
        shelude += '%s %s %s %s\n' % (row[1], row[2], row[kurs2[msg['text']]].replace('\n', ' / '), row[kurs2[msg['text']] + 1].replace('\n', ' / '))
print(shelude)
